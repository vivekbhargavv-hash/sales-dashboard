...
export default function MonthlyClosures({ monthlyClosures, monthlySummary }) {
  ...
  // Sort monthlyClosures: by month asc, within month by vehicles desc
  const sortedClosures = useMemo(() => {
    if (!monthlyClosures || monthlyClosures.length === 0) return []
    return [...monthlyClosures].sort((a, b) => {
  ...
  // Track which months have already shown their label (for merge-cell effect)
        ...
        <div className="overflow-x-auto pb-1">
          <div className="flex gap-3 min-w-max">
            {monthlySummary.map((item) => (
              <div
                key={item.month}
                className={`${tw.card} rounded-xl px-5 py-4 min-w-[140px] flex-shrink-0`}
              >
                ...
                <div className="space-y-1.5">
                  <div>
                    <p className={`text-[11px] ${tw.textMuted}`}>Deals</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className={`${tw.divider}`}>
                  <th className="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wide text-green-600 whitespace-nowrap">
                    Month
                  </th>
                  <th className="text-left py-3 px-4 text-xs font-semibold uppercase tracking-wide text-green-600 whitespace-nowrap">
                    Client
                  ...
                  <th className="text-right py-3 px-4 text-xs font-semibold uppercase tracking-wide text-green-600 whitespace-nowrap">
                    Vehicles
                  </th>
                </tr>
              </thead>
